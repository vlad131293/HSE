...
def df_info(df: pd.DataFrame, top: int=1) -> pd.DataFrame:
...
def binarize_num(X: pd.DataFrame, n_bins=10) -> pd.DataFrame:
    disc = KBinsDiscretizer(n_bins=n_bins, encode='onehot', strategy='quantile')
    result = []
    for col in X.columns:
        new_columns = [f'{col}_{i}' for i in range(n_bins)]
        dummies = disc.fit_transform(X[col].values.reshape(-1, 1))
        temp = pd.DataFrame(dummies.toarray(), columns=new_columns)
        result.append(temp)
    return pd.concat(result, axis=1).astype(bool)


def binarize_cat(X: pd.DataFrame) -> 'pd.DataFrame[bool]':
...
def predict_with_generators_improved(
    x: np.array, X_train: np, y_train: np.array,
    min_cardinality=1) -> bool:
    ...
    n_counters_pos = 0
    intr_pos = (x & X_pos)
    for inrt_x in intr_pos:
        k = len(inrt_x.nonzero()[0])
        if k < min_cardinality:
            continue  
        temp = X_neg[:, inrt_x.nonzero()[0]]
        n_counters_pos += (temp.sum(axis=1) == k).sum()

    n_counters_neg = 0
    intr_neg = (x & X_neg)
    for inrt_x in intr_neg:
        k = len(inrt_x.nonzero()[0])
        if k < min_cardinality:
            continue
        temp = X_pos[:, inrt_x.nonzero()[0]]
        n_counters_neg += (temp.sum(axis=1) == k).sum()
    
    perc_counters_pos = n_counters_pos / len(X_pos)
    perc_counters_neg = n_counters_neg / len(X_neg)

    return perc_counters_pos < perc_counters_neg

def predict_array_improved(
        X_bin: np.array, y: np.array,
        n_train: int, update_train: bool = True, use_tqdm: bool = False,
        predict_func=predict_with_generators
):
    
    for i, x in tqdm(
        enumerate(X_bin[n_train:]),
        initial=n_train, total=len(X_bin),
        desc='Predicting step by step',
        disable=not use_tqdm,
    ):
        n_trains = n_train + i if update_train else n_train
        yield predict_func(x, X_bin[:n_trains], y[:n_trains])

def predict_with_generators_modified(
    x: np.array, X_train: np, y_train: np.array,
    min_cardinality=1) -> bool:
    ...
    return perc_counters_neg / perc_counters_pos
